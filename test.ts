...
describe("Project", function () {
  ...
  it("has the basic", function () {
    ...
    let rsvp = project
      .addDependency("ember-cli", "3.1.1", (cli) =>
        cli.addDependency("console-ui", "3.3.3")
      )
    ...
    expect(rsvp.root).to.eql(
      path.normalize(`${project.root}/rsvp/node_modules/ember-cli/node_modules`)
    );
    expect(source.root).to.eql(
      path.normalize(`${project.root}/rsvp/node_modules`)
    );
    expect(rsvp.baseDir).to.eql(
      path.normalize(
        `${project.root}/rsvp/node_modules/ember-cli/node_modules/rsvp`
      )
    );
    expect(source.baseDir).to.eql(
      path.normalize(`${project.root}/rsvp/node_modules/ember-source`)
    );

    expect(read(`${project.root}/rsvp/index.js`)).to.eql(
    ...
    expect(
      read(`${project.root}/rsvp/node_modules/ember-source/index.js`)
    ).to.contain(`module.exports`);
    expect(
      require(`${project.root}/rsvp/node_modules/ember-source/index.js`)
    ).to.eql({});

    expect(
      readJSON(`${project.root}/rsvp/node_modules/ember-source/package.json`)
    ).to.eql({
      name: "ember-source",
      version: "3.1.1",
      keywords: [],
    ...
    expect(
      readJSON(`${project.root}/rsvp/node_modules/ember-cli/package.json`)
    ).to.eql({
      name: "ember-cli",
      version: "3.1.1",
      keywords: [],
    ...
    expect(
      read(
        `${project.root}/rsvp/node_modules/ember-cli/node_modules/console-ui/index.js`
      )
    ).to.contain(`module.exports`);
    expect(
      require(`${project.root}/rsvp/node_modules/ember-cli/node_modules/console-ui/index.js`)
    ).to.eql({});

    expect(
      read(`${project.root}/rsvp/node_modules/@ember/ordered-set/index.js`)
    ).to.contain(`module.exports`);
    expect(
      require(`${project.root}/rsvp/node_modules/@ember/ordered-set/index.js`)
    ).to.eql({});

    expect(
      readJSON(
        `${project.root}/rsvp/node_modules/ember-cli/node_modules/console-ui/package.json`
      )
    ).to.eql({
      name: "console-ui",
      version: "3.3.3",
  ...
  it("supports default version", function () {
    ...
    expect(JSON.parse(input.toJSON("package.json") as string)).to.have.property(
      "version",
      "0.0.0"
    );
  });
  ...
  it("supports removing packages", function () {
    ...
    input.addDependency("rsvp", "4.4.4", (rsvp) =>
      rsvp.addDependency("mkdirp", "4.4.4")
    );
    input.addDevDependency("omg", "4.4.4", (omg) =>
      omg.addDependency("fs-extra", "5.5.5.")
    );
  ...
  it("requires name and version", function () {
    ...
    expect(() => new P("rsvp", null)).to.throw(
      /rsvp's package.json is missing a version/
    );
  ...
  it("it supports deep cloning", function () {
    ...
    expect(input.dependencies().map((x) => x.name)).to.contain(
      "-no-such-package-"
    );
    expect(output.dependencies().map((x) => x.name)).to.not.contain(
      "-no-such-package-"
    );
  });
